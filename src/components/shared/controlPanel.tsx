        ...
        <div className='flex justify-center'>
            <div className='w-152.5 h-27.5 rounded-lg shadow-lg border border-gray-300 bg-white flex flex-col px-4 items-center justify-center'>
                    ...
                    <div className='flex gap-2 items-center'>
                        ...
                        {['1x', '2x', '5x'].map((speedVal) => (
                            <button 
                                ...
                                onClick={() => controller?.setSpeed?.(speedVal as "1x" | "2x" | "5x")}
                                disabled={!controller}
                            >
                                {speedVal}
                            </button>
                ...
                <div className='flex flex-row gap-2'>
                    <button onClick={controller?.prevStep} disabled={!controller} type="button" className='w-30 h-10 p-2 border border-gray-300 rounded-lg flex items-center justify-center hover:bg-gray-200'>
                        <ChevronFirst />
                        Prev Step
                    </button>
                    <button onClick={controller?.nextStep} disabled={!controller} type="button" className='w-30 h-10 p-2 border border-gray-300 rounded-lg flex items-center justify-center hover:bg-gray-200'>
                        Next Step
                        <ChevronLast />
                    </button>
                    <button onClick={controller?.skipBack} disabled={!controller} type="button" className='w-30 h-10 p-2 border border-gray-300 rounded-lg flex items-center justify-center hover:bg-gray-200'>
                        <ChevronsLeft />
                        Skip Back
                    </button>
                    <button onClick={controller?.skipForward} disabled={!controller} type="button" className='w-35.5 h-10 p-2 border border-gray-300 rounded-lg flex items-center justify-center hover:bg-gray-200'>
                        Skip Forward
                        <ChevronsRight />
                    </button>
                </div>
