...
type ExplainAlgoProps = {
    isOpen: boolean;
    onToggle: () => void;
    explanation: string;
};

export default function ExplainAlgo({ isOpen, onToggle, explanation }: ExplainAlgoProps) {
    ...
    return (
        <>
            <button
                className={`border-b border-black flex items-center justify-between w-full transition-all duration-300 ease-in-out ${isExplain ? "bg-gray-200 h-12" : "bg-white"
                    }`}
            ...
            >
                <div className="flex items-center">
                    <div
                        className={`bg-blue-600 w-2 h-12 transition-all duration-300 ease-in-out z-50 ${isExplain ? "" : "hidden opacity-0"
                            }`}
                    ></div>
            ...
            <div
                className={`overflow-hidden transition-all duration-600 ease-in-out ${isExplain ? "max-h-32 opacity-100" : "max-h-0 opacity-0"
                    }`}
            >
