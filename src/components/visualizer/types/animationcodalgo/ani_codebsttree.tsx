...
export default function CodeBSTTreeView({
  title = "Code",
  code = "",
  language = "python",
  tutorialMode = false,
  ...
  const defaultCode = useMemo(() => {
    const action = treeAction ?? "bst";
    if (action === "bst-insert") {
      return `ALGORITHM BSTInsert(root, value)
    node = root
    WHILE node IS NOT null DO
        ...
        END IF
    END WHILE
    INSERT new node at empty position
END ALGORITHM`;
    }
    if (action === "bst-search") {
      return `ALGORITHM BSTSearch(root, value)
    node = root
    WHILE node IS NOT null DO
        IF value == node.value THEN
            RETURN FOUND
...
END ALGORITHM`;
    ...
    if (action === "bst-remove") {
      return `ALGORITHM BSTRemove(root, value)
    node = root
    WHILE node IS NOT null AND node.value != value DO
        IF value < node.value THEN
            node = node.left
        ELSE
            node = node.right
...
END ALGORITHM`;
    ...
    return `ALGORITHM BinarySearchTree(root)
