...
export default function CodeDijkstraGraph({
  title = "Code",
  code = "",
  language = "python",
  tutorialMode = false,
  ...
  const defaultCode = useMemo(() => {
    return `ALGORITHM Dijkstra(graph, start, target)
    FOR each node DO
        dist[node] = infinity
        prev[node] = null
    END FOR
    dist[start] = 0
    SET visited = empty
    WHILE there exists an unvisited node DO
        ...
        END FOR
    END WHILE
    RETURN shortest path using prev[]
END ALGORITHM`;
  }, []);
