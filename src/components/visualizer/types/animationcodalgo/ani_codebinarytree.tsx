...
export default function CodeBinaryTreeView({
  title = "Code",
  code = "",
  language = "python",
  tutorialMode = false,
  ...
  const defaultCode = useMemo(() => {
    const action = treeAction ?? "binary-tree";
    if (action === "bt-traversal-inorder") {
      return `ALGORITHM InorderTraversal(node)
...
END ALGORITHM`;
    ...
    if (action === "bt-traversal-preorder") {
      return `ALGORITHM PreorderTraversal(node)
    IF node IS null THEN
        RETURN
    END IF
    VISIT node
...
END ALGORITHM`;
    ...
    if (action === "bt-traversal-postorder") {
      return `ALGORITHM PostorderTraversal(node)
    IF node IS null THEN
        RETURN
    END IF
...
END ALGORITHM`;
    ...
    if (action === "bt-insert") {
      return `ALGORITHM BTInsert(root, value)
...
END ALGORITHM`;
    ...
    if (action === "bt-search") {
      return `ALGORITHM BTSearch(root, value)
    IF root IS null THEN
        RETURN NOT_FOUND
    END IF
    QUEUE = [root]
...
END ALGORITHM`;
    ...
    if (action === "bt-remove") {
      return `ALGORITHM BTRemove(root, value)
    SEARCH target by level-order
    IF target NOT_FOUND THEN
        RETURN
    END IF
    FIND deepest rightmost node
    REPLACE target.value with last.value
    DELETE last node
END ALGORITHM`;
    }

    // Generic BT operations used by insert/remove/search animations
    return `ALGORITHM BinaryTreeOperation(root, value)
    // operations are shown step-by-step by the animation
    QUEUE = [root]
    WHILE QUEUE is not empty DO
        node = POP_FRONT(QUEUE)
        IF node matches target / has empty child THEN
            UPDATE links
            RETURN
        END IF
        PUSH children into QUEUE
