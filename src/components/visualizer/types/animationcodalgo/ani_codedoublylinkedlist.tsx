...
export default function CodeDoublyLinkedList({
  title = "Code",
  code = "",
  language = "python",
  tutorialMode = false,
  ...
  const defaultCode: string = `ALGORITHM DoublyLinkedListOperation(head, operation, index, value)
    IF operation == "insert" THEN
        newNode = Node(value)
        current = node at index - 1
    ...
    ELSE IF operation == "delete" THEN
        ...
        IF current.prev != NULL THEN current.prev.next = current.next
        IF index == 0 THEN head = current.next
    END IF
END ALGORITHM`;
