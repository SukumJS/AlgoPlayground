...
export default function CodeAVLTreeView({
  title = "Code",
  code = "",
  language = "python",
  tutorialMode = false,
  ...
  const defaultCode = useMemo(() => {
    const action = treeAction ?? "avl";
    if (action === "avl-insert") {
      return `ALGORITHM AVLInsert(root, value)
    INSERT value like BST
    node = inserted node
    WHILE node IS NOT null DO
        UPDATE height(node)
        bf = balanceFactor(node)
        IF bf is -2 or +2 THEN
            ROTATE to rebalance (LL / RR / LR / RL)
            BREAK
        END IF
        node = node.parent
    END WHILE
END ALGORITHM`;
    ...
    if (action === "avl-search") {
      return `ALGORITHM AVLSearch(root, value)
    node = root
    WHILE node IS NOT null DO
...
END ALGORITHM`;
    ...
    if (action === "avl-remove") {
      return `ALGORITHM AVLRemove(root, value)
    REMOVE value like BST
    node = parent of deleted position
    WHILE node IS NOT null DO
        UPDATE height(node)
        bf = balanceFactor(node)
        IF bf is -2 or +2 THEN
