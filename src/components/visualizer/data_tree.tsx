...
// let id = 0;
const getId = () =>
  `dndnode_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
...
function Data_tree({
  ...
  const [avlRoot, setAVLRoot] = useState<AVLTreeNode | null>(() => {
    ...
    const numericNodes = currentNodes.filter(
      (n) => !isNaN(parseInt(n.data.label)),
    );
  ...
  useEffect(() => {
    if (isBT && !isAnimating) {
      ...
      setTimeout(() => {
        setBTRoot(
          rebuildBTFromReactFlow(JSON.parse(nodesStr), JSON.parse(edgesStr)),
        );
      }, 0);
    }
        ...
        return {
          highlightedNodes: nodes.map((n: RFNode) => ({
            ...
            data: {
              ...
              highlightColor: highlightedNodeIds.has(n.id)
                ? resolvedColor
                : undefined,
            },
          })),
  ...
  const handleAVLRebalance = useAVLRebalanceHandler({
    avlRoot,
    setAVLRoot,
    rf,
    animationCallbacks,
  ...
  // BST Handlers
  const { handleInsert: bstInsert } = useBSTInsertHandler({
    bstRoot,
    setBSTRoot,
    setNodes,
    setEdges,
  ...
  // BT Handlers
  const { handleInsert: btInsert } = useBTInsertHandler({
    btRoot,
    setBTRoot,
    nodes: rf.getNodes(),
    edges: rf.getEdges(),
  ...
  // Heap Handlers
  const { handleInsert: heapInsert } = useHeapInsertHandler({
    heapRoot,
    setHeapRoot,
    isMinHeap,
    nodes: rf.getNodes(),
  ...
  const { handleSearch: heapSearch } = useHeapSearchHandler({
    heapRoot,
    setNodes,
    setEdges,
    setDescription: animationCallbacks.setDescription,
  ...
  const { handleRemove: heapRemove } = useHeapRemoveHandler({
    heapRoot,
    setHeapRoot,
    isMinHeap,
    setNodes,
  ...
  useEffect(() => {
    onRebalanceReady?.(handleAVLRebalance);
  }, [handleAVLRebalance, onRebalanceReady]);
  ...
  useEffect(() => {
    onBTRebalanceReady?.(handleBTRebalance);
  }, [handleBTRebalance, onBTRebalanceReady]);
  useEffect(() => {
    onAutoInsertReady?.(btInsert);
  }, [btInsert, onAutoInsertReady]);
  ...
  useEffect(() => {
    onHeapRebalanceReady?.(handleHeapRebalance);
  }, [handleHeapRebalance, onHeapRebalanceReady]);
  ...
  const handleTrashDelete = useCallback(
    (nodeId: string, value: number) => {
      ...
      if (isHeap) {
        setHeapRoot((prev) =>
          prev ? removeHeap(prev, value, isMinHeap).root : null,
        );
      }
    },
  ...
  useEffect(() => {
    onTrashDeleteReady?.(handleTrashDelete);
  }, [handleTrashDelete, onTrashDeleteReady]);
