...
export function generateBinarySearchSteps(
  ...
  while (left <= right) {
    ...
    if (midValue === target) {
      // เจอเป้าหมาย เปลี่ยนเป็นสีเขียว แล้วจบการทำงาน
      currentNodes[mid].data.status = "found";
      steps.push(JSON.parse(JSON.stringify(currentNodes)));
      break;
    } else if (midValue < target) {
      // ค่าตรงกลาง "น้อยกว่า" เป้าหมาย แปลว่าฝั่งซ้ายทั้งหมด (รวมถึงตัวมันเอง) ใช้ไม่ได้แล้ว ตัดทิ้งให้เป็นสีเทา!
      ...
      // ค่าตรงกลาง "มากกว่า" เป้าหมาย แปลว่าฝั่งขวาทั้งหมด (รวมถึงตัวมันเอง) ใช้ไม่ได้แล้ว ตัดทิ้งให้เป็นสีเทา!
