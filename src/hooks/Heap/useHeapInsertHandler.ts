...
export function useHeapInsertHandler(params: {
    ...
    const handleInsert = useCallback((value: number) => {
        ...
        // Step 1: Highlight BFS traversal path
        if (path.length > 0) {
            ...
            path.forEach((id, idx) => {
                controller.scheduleStep(() => {
                    ...
                    const highlightedEdges = (origRF.edges as RFEdge[]).map((e: RFEdge) => ({
                        ...e,
                        style: path.slice(0, idx).some(pid => e.source === pid || e.target === pid)
        ...
        // Step 2: Show inserted node (green)
        globalOffset++;
        controller.scheduleStep(() => {
            ...
            // Before sift-up, highlight the newly inserted node
            ...
            setDescription(`Inserted ${value}. ${siftPath.length > 0 ? 'Sifting up...' : 'Heap property maintained!'}`);
        }, animationSpeed * globalOffset);

        // Step 3: Show sift-up swaps (yellow highlights)
        siftPath.forEach((swapId, idx) => {
            ...
            controller.scheduleStep(() => {
                ...
                const highlighted = (rfNodes as RFNode[]).map((n: RFNode) => ({
                    ...
                    data: {
                        ...n.data,
                        isHighlighted: n.id === swapId,
                        highlightColor: n.id === swapId ? '#F7AD45' : undefined,
                    },
                }));
                setNodes(highlighted);
                setEdges(rfEdges as RFEdge[]);
                setDescription(`Sift up: swapped to position ${idx + 1}`);
            }, animationSpeed * globalOffset);
        });
